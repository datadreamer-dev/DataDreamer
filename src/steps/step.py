...
class StepOutput(UserDict):
    def __init__(self, output_names: Tuple[str], *args, **kwargs):
...
# Generator function of batches
    ...
    def output(
        self,
        value: Union[
            Dataset, IterableDataset, Dict[str, Iterable[Any]], List[Any], Tuple[Iterable[Any]], Callable[[], Generator[Dict[str, Iterable[Any], List[Any], Tuple[Any]], None, None]]
        ],
    ):
        ...
        if isinstance(value, dict) and set(self.output_names) == set(value.keys()):
            if any([callable(v) for v in value.values()]):
                ...
                iters = [(value[k]() if callable(value[k]) else iter(value[k])) for k in self.output_names]
                rows = zip(*iters)
        ...
        if (isinstance(value, Dataset) or isinstance(value, IterableDataset)) and set(value.column_names) != set(
            self.output_names
        ):
            raise AttributeError(
        ...
        if callable(value):
            ...
            try:
                ...
                if isinstance(first_row, dict) and set(self.output_names) != set(value.keys()):
                    raise f"Expected {self.output_names} dict keys from generator function instead of {list(first_row.keys())}."
                elif type(first_row) in [list, tuple]:
                    if len(first_row) != len(self.output_names):
                        raise AttributeError(
                            f"Expected {len(self.output_names)} outputs ({self.output_names}) from generator function"
                        )
                    def to_dict_generator_wrapper():
                        for row in value():
                            yield {k: v for k, v in zip(self.output_names, row)}
