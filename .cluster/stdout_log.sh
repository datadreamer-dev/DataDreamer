...
# Fetch the stdout log
if [ -z "$JOB_NAME" ]; then
    echo "Fetching the stdout log of the last job..." 1>&2
    if [ -f "./output/named/_latest/job/.array" ]; then
        if [ -z "$TASK_ID" ]; then
            ...
            export LOG_PATH=./output/named/_latest/$TASK_ID/stdout.out
        fi
    else
        export LOG_PATH=./output/named/_latest/stdout.out
    fi
    if [ -f "$LOG_PATH" ]; then
        cat $LOG_PATH
    else
    ...
    echo "Fetching the stdout log of the '$JOB_NAME' job..." 1>&2
    if [ -f "./output/named/$JOB_NAME/job/.array" ]; then
        if [ -z "$TASK_ID" ]; then
            echo "You must provide the task ID with a colon (:<task_id>) since this job was run as a job array." 1>&2
            exit 1
        else
            export LOG_PATH=./output/named/$JOB_NAME/$TASK_ID/stdout.out
        fi
    else
        export LOG_PATH=./output/named/$JOB_NAME/stdout.out
    fi
    if [ -f "$LOG_PATH" ]; then
        cat "$LOG_PATH"
    else
